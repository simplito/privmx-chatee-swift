...
public class APIClient {
  ...
  public init(
    baseURL: String, sessionManager: Session = .default, defaultHeaders: [String: String] = [:],
  ...
  public func makeRequest<T>(
    ...
    requestBehaviour.validate(urlRequest) { result in
      ...
      case .success(let urlRequest):
        let networkRequest = self.makeNetworkRequest(
          request: request, urlRequest: urlRequest, requestBehaviour: requestBehaviour,
  ...
  private func makeNetworkRequest<T>(
    request: APIRequest<T>, urlRequest: URLRequest, requestBehaviour: RequestBehaviourGroup,
    completionQueue: DispatchQueue, complete: @escaping (APIResponse<T>) -> Void
  ) -> Request {
              ...
              func recursiveAppend(formData: MultipartFormData, key: String, value: Any) {
                ...
                case let dictionary as [String: Any]:
                  for (dictionaryKey, dictionaryValue) in dictionary {
                    recursiveAppend(
                      formData: formData, key: "\(key)[\(dictionaryKey)]", value: dictionaryValue)
                  }
  ...
  private func handleResponse<T>(
    ...
    dataResponse: AFDataResponse<Data>, completionQueue: DispatchQueue,
    complete: @escaping (APIResponse<T>) -> Void
  ) {
    ...
    let response = APIResponse<T>(
      ...
      urlResponse: dataResponse.response, data: dataResponse.data, metrics: dataResponse.metrics)
...
extension APIRequest {
  ...
  public func createURLRequest(baseURL: URL, encoder: RequestEncoder = JSONEncoder()) throws
    -> URLRequest
  {
