...
open class ListModel<T>: ObservableObject where T: Identifiable & Hashable & Sendable {
	...
	@Published public var seenAll = true
	@Published public var seenAllOthers = true

	public var loader:
		((Int64, Int64, String, @escaping (@Sendable (Int64, [T]) -> Void)) -> Void)?
	...
	public func add(element: T) {
		...
		self.data.appendElementsNotExistent(
			from: [element],
			distinguishedBy: self.thesame ?? { a, b in return a.id == b.id },
	...
	public func loadNext() {
		Task{
			await self.load(skip: loaded)
		}
	}

	public func loadNew() {
		Task {
			await self.load(skip: 0)
		}
	}
	public func reload() {
		Task {
	...
	public func getFirstFileFromRepository(e: @escaping @Sendable (T) -> Void)  async{
	...
	public func getFilesFromRepository(first: Int64, e: @escaping @Sendable ([T]) -> Void) async {
	...
	public func load(skip: Int64? = nil) async {
